...
class TextProcessor:
    ...
    def process_text(self, text, vocabulary=None):
        ...
        # 3b. Corregir mayúsculas erróneas (en mitad de frase por pausas)
        text = self.fix_erroneous_caps(text)
    ...
    def fix_erroneous_caps(self, text):
        """Corrige mayúsculas que aparecen en mitad de una frase, a menudo por pausas en el dictado"""
        if not text:
            return text
        ...
        def repl(match):
            ...
            # Si es el inicio absoluto del texto, no tocar (capitalize_sentences lo hará)
            if not prefix:
                return word
                
            # Si viene precedido por puntuación de final de frase o salto de línea, mantener mayúscula
            if re.search(r'[.!?\n]', prefix):
                return match.group(0)
            
            # Si es un acrónimo (todo mayúsculas, >1 letra), mantener (ej: TAC, RM, HTA)
            if word.isupper() and len(word) > 1:
                return match.group(0)
                
            # Si viene después de dos puntos o paréntesis, solemos mantener la intención original
            if re.search(r'[:(]', prefix):
                return match.group(0)
                
            # En cualquier otro caso (espacio, coma, punto y coma en medio de frase), poner en minúscula
            return prefix + word[0].lower() + word[1:]
            
        # Busca palabras que empiezan por Mayúscula y siguen con minúsculas
        # El prefijo captura el espacio o signo previo
        return re.sub(r'(^|[\s,;]+)([A-ZÁÉÍÓÚÜÑ][a-záéíóúüñ]*)', repl, text)
