...
class DictadoRadiologicoApp:
    def __init__(self, root):
        ...
        self.last_toggle_time = 0 # Para evitar dobles pulsaciones rápidas
        ...
        self.root.bind('<Control-i>', lambda e: self.toggle_recording())
        self.root.bind('<Control-I>', lambda e: self.toggle_recording())
        self.root.bind('<Control-Shift-KeyPress-i>', lambda e: self.toggle_recording())
        self.root.bind('<Control-Shift-KeyPress-I>', lambda e: self.toggle_recording())
    ...
    def setup_theme(self):
        ...
        style.map('TCombobox', 
                  fieldbackground=[('readonly', COLORS['bg_secondary']), ('disabled', COLORS['bg_primary'])],
                  foreground=[('readonly', COLORS['text_primary']), ('disabled', COLORS['text_secondary'])],
                  arrowcolor=[('readonly', COLORS['accent']), ('disabled', COLORS['text_secondary'])])
        
        # Eliminar el borde azul por defecto al enfocar
        style.layout('TCombobox', style.layout('TCombobox')) # Reconfigurar layout si fuera necesario
    ...
    def setup_ui(self):
        ...
        # Selector de Proveedor ELIMINADO de aquí (ahora en status_frame)
        ...
        # Selector de Proveedor (AI) - REUBICADO PARA EVITAR RECORTES
        ai_sel_frame = tk.Frame(status_frame, bg=COLORS['bg_secondary'])
        ai_sel_frame.pack(side=tk.LEFT, padx=(30, 0))
        
        tk.Label(ai_sel_frame, text="IA de Transcripción:",
                 bg=COLORS['bg_secondary'],
                 fg=COLORS['text_secondary'],
                 font=('Segoe UI', 10, 'bold')).pack(side=tk.LEFT, padx=(0, 5))
        
        self.provider_combo = ttk.Combobox(ai_sel_frame, 
        ...
        audio_container.columnconfigure(1, weight=0, minsize=330)  # Columna derecha: imagen (AJUSTADO A 320px PARA LLENAR EL PANEL)
        ...
        if not os.path.exists(batman_folder):
            # Fallback a la carpeta donde está el script
            batman_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'batman_images')
        ...
        if batman_paths:
            try:
                if PIL_AVAILABLE:
                    ...
                    for img_path in batman_paths:
                        try:
                            ...
                            # Tamaño optimizado: ancho máx 320px, alto máx 220px (equilibrio entre visibilidad y espacio)
                            max_w, max_h = 320, 220
                            img_w, img_h = img.size
                            
                            # Calcular escala para encajar en 320x220 manteniendo aspecto
                ...
                if self.batman_images:
                    # Crear frame con fondo del tema y TAMAÑO OPTIMIZADO (324x224 para imagen de 320x220)
    ...
    def toggle_recording(self, event=None):
        """Inicia o detiene la grabación con protección contra rebotes"""
        current_time = time.time()
        # Cooldown de 0.6 segundos para evitar arranques/paradas accidentales
        if current_time - self.last_toggle_time < 0.6:
            return
    ...
    def _setup_comic_section(self):
        ...
        try:
            ...
            if PIL_AVAILABLE:
                images = []
                img_paths = [
                    os.path.join(batman_folder, 'joker.jpg'),
                    os.path.join(batman_folder, 'batmanneal.jpg'),
                ]
                for p in img_paths:
                    if os.path.exists(p):
                        from PIL import Image as PILImage, ImageTk as PILImageTk
